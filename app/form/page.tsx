...
function Page() {
  // 1. Setup state for form data and submission status
  const [formData, setFormData] = useState({ name: '', country: '', email: '', phone: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const WEB_APP_URL = process.env.NEXT_PUBLIC_GOOGLE_SCRIPT_URL || ''; // <--- REPLACE THIS
    ...
    try {
      ...
      // setFormData({ name: '', country: '', email: '', phone: '' });
    } catch (error) {
        ...
        <div className="relative p-6 sm:p-8 md:p-14 border-b lg:border-b-0 lg:border-r border-white/10">
          ...
          <form onSubmit={handleSubmit} className="mt-8 md:mt-10 space-y-5 md:space-y-6">
            ...
            <button
              ...
              className="group relative mt-4 w-full overflow-hidden rounded-2xl p-[1px] transition-all duration-500 hover:scale-[1.01] disabled:opacity-70 disabled:hover:scale-100"
            >
              <div
              ...
              <div className="relative flex items-center justify-center gap-3 rounded-2xl bg-black/80 px-6 sm:px-8 py-4 sm:py-5 backdrop-blur-3xl">
                ...
                <span className="relative z-10 text-xs sm:text-sm font-black uppercase tracking-[0.24em] sm:tracking-[0.3em] text-white text-center">
                  {status === 'loading' ? 'Sending...' : status === 'success' ? 'Request Sent' : 'Initialize Request'}
                </span>
              </div>
            </button>
          </form>
